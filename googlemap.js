...
function render_map() {
    ...
    if (!ValidURL(domain)) {
        ...
        if (domain.indexOf("https") > -1) domain = domain.replace('https://', '1');
        if (domain.indexOf("http") > -1) domain = domain.replace('http://', '0');
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                document.getElementById("blokk").style.display = "block"
                document.getElementById("server").textContent = data[0].org
            }
        });
...
function load_map(servers) {
    ...
    servers.forEach(function (server) {
        ...
        // console.log('Server: ', server.serv);
